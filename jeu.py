...
class Jeu:
    ...
    def reinitialiser(self):
        ...
        self.gestionnaire_vague.configurer_contexte(self.continent, self.niveau)
        ...
        # Système d'indicateur de direction des mobs
        self._indicateur_direction_actif = False
        self._nombre_mobs_spawnes_indicateur = 0
    ...
    def gerer_clic(self, clic):
        ...
        if action_niveau == "niveau_suivant":
            self.niveau = min(7, self.niveau + 1)  # 7 niveaux maintenant
            self.reinitialiser()
            return
    ...
    def lancer_nouvelle_vague(self):
        ...
        # Activer l'indicateur de direction des mobs avant de lancer la vague
        if len(CHEMIN) > 1:
            self._indicateur_direction_actif = True
            self._nombre_mobs_spawnes_indicateur = 0
            self._position_indicateur = CHEMIN[0]  # Position de spawn
            # Calculer la direction initiale (vers le deuxième point du chemin)
            direction_x = CHEMIN[1][0] - CHEMIN[0][0]
            direction_y = CHEMIN[1][1] - CHEMIN[0][1]
            # Normaliser la direction pour avoir une longueur de 10 pixels
            longueur = (direction_x**2 + direction_y**2)**0.5
            if longueur > 0:
                self._direction_indicateur = (
                    int(direction_x * 10 / longueur),
                    int(direction_y * 10 / longueur)
                )
            else:
                self._direction_indicateur = (10, 0)  # Direction par défaut vers la droite
    ...
    def mettre_a_jour(self, delta_temps):
        ...
        if not self.en_attente_lancement_vague and not self.ecran_fin_vague.visible and not self.fenetre_marche.visible:
            ...
            if self._indicateur_direction_actif:
                mobs_avant = len(self.liste_ennemis)
                self.gestionnaire_vague.mettre_a_jour(delta_temps, self.liste_ennemis, CHEMIN)
                mobs_apres = len(self.liste_ennemis)
                self._nombre_mobs_spawnes_indicateur += (mobs_apres - mobs_avant)
                
                # Désactiver l'indicateur après le spawn du 4ème mob
                if self._nombre_mobs_spawnes_indicateur >= 4:
                    self._indicateur_direction_actif = False
            else:
                self.gestionnaire_vague.mettre_a_jour(delta_temps, self.liste_ennemis, CHEMIN)
            for ennemi in self.liste_ennemis:
    ...
    def dessiner(self):
        ...
        if self._indicateur_direction_actif:
            pos_x, pos_y = self._position_indicateur
            dir_x, dir_y = self._direction_indicateur
            ...
            pygame.draw.line(self.fenetre, (255, 50, 50), 
            ...
            pygame.draw.polygon(self.fenetre, (255, 255, 255),
                              [(arrivee_x, arrivee_y), (pointe1_x, pointe1_y), (pointe2_x, pointe2_y)])
            pygame.draw.polygon(self.fenetre, (255, 50, 50),
        ...
        symbole = self.police_hud.render("¤", True, couleur_texte)
        ...
        self.fenetre.blit(symbole, pos_symbole)
        ...
        if self._timer_message_victoire > 0:
            ...
            alpha_victoire = min(255, int(self._timer_message_victoire * 60))
            surf_victoire = police_victoire.render(self._message_victoire, True, (255, 215, 0))
            surf_victoire.set_alpha(alpha_victoire)
            
            # Effet de pulsation
            pulsation = _math.sin(self._timer_message_victoire * 8) * 5
            pos_x = self._position_message_victoire[0] - surf_victoire.get_width() // 2 + pulsation
            pos_y = self._position_message_victoire[1] - surf_victoire.get_height() // 2
            
            # Ombre du texte
            surf_ombre = police_victoire.render(self._message_victoire, True, (100, 50, 0))
            surf_ombre.set_alpha(alpha_victoire // 2)
            self.fenetre.blit(surf_ombre, (pos_x + 3, pos_y + 3))
    ...
    def _lancer_animation_victoire_finale(self):
        """
        Explication de ce que fais la fonction : Cette fonction lance l'animation de victoire finale pour le niveau 8 démoniaque.
        Les entrées : Cette fonction ne demande pas de paramètre direct.
        Le résultat : Crée une animation spectaculaire de victoire.
        """
        import random as _rand
        import math as _math
        
        # Créer plusieurs cercles d'expansion dorés depuis le centre de l'écran
        centre_x = largeur_ecran // 2
        centre_y = hauteur_ecran // 2
        
        # Grande explosion centrale
        self._ajouter_effet((centre_x, centre_y), (255, 215, 0), 80, 3.0)
        
        # Cercles d'ondes qui s'étendent
        for i in range(5):
            delai = i * 0.3
            rayon = 50 + i * 30
            # Créer des effets en cercle
            for angle in range(0, 360, 30):
                x = centre_x + int(rayon * _math.cos(_math.radians(angle)))
                y = centre_y + int(rayon * _math.sin(_math.radians(angle)))
                couleur = (_rand.randint(200, 255), _rand.randint(150, 255), _rand.randint(0, 100))
                self._ajouter_effet((x, y), couleur, 15, 2.0 - i * 0.2)
        
        # Étoiles filantes depuis les coins
        for coin in [(0, 0), (largeur_ecran, 0), (0, hauteur_ecran), (largeur_ecran, hauteur_ecran)]:
            for i in range(3):
                x = coin[0] + _rand.randint(-50, 50)
                y = coin[1] + _rand.randint(-50, 50)
                self._ajouter_effet((x, y), (255, 255, 100), 25, 2.5)
        
        # Texte "VICTOIRE !" flottant
        self._message_victoire = "VICTOIRE FINALE !"
        self._timer_message_victoire = 5.0
        self._position_message_victoire = (centre_x, centre_y - 100)
        
        # Screen shake prolongé pour l'effet dramatique
        self._shake_timer = 2.0
        self._shake_amplitude = 15
